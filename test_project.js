class User {
  ...
  substractBalance(amount) {
    if (this.balance - amount < 0) {
      throw new Error(`Ooops...Sorry, ${this.name} don't have enough money`);
    }
...
class Product {
  ...
  substractAmount() {
    if ((this.amountOfProducts - 1) < 0) {
      throw new Error(`Ooops...${this.name} is sold`);
    }
...
const admin = new Admin('Bodya', 1000);
...
console.log('Tipok added banana, milk, apple to cart');
console.log('Tipok created order: ', user.cart.checkout());
console.log('Tipok\'s balance:', user.balance);